...
class Yeelight extends module {
    ...
    function usual(&$out) {
        if ($this->ajax) {
            ...
            if ($op=='process') {
                ...
                $this->processMessage($header,$peer,$packet);
            }
        }
        $this->admin($out);
    }
    ...
    /**
     * Обработка мультикаста от устройств Yeelight
     * @param type $header - заголовок сообщения
     * @param type $peer - адрес источника
     * @param type $packet - содержимое пакета в виде массива ключ=>значение
     */
    function processMessage($header,$peer,$packet) {
        #TODO обработка 
    }
        ...
        foreach ($bulbList_prop as $bulb) {
            ...
            if (!$searhID) {
                if ($name) {
                    ...
                    if ($model=="color"||$model=="color1") {
                        $objDescription=array('Цветная лампочка');
                        $rec=SQLSelectOne("SELECT ID FROM classes WHERE TITLE LIKE '".DBSafe("Yeelight")."'");
                        if (!$rec['ID']) {
                            $rec=array();
                            $rec['TITLE']=$objName;
                    ...
                    if ($model=="mono"||$model=="mono1") {
                        $objDescription=array('Белая лампочка');
                        $rec=SQLSelectOne("SELECT ID FROM classes WHERE TITLE LIKE '".DBSafe("Yeelight")."'");
                        if (!$rec['ID']) {
                            $rec=array();
                            $rec['TITLE']=$objName;
