...
$socketserver=new YeelightSocketServer();
$socketserver->run();
while (1) {
    ...
    if ((time()-$latest_check)>$checkEvery) {
        ...
        $pkt=$socketserver->getPacket();
        $url=BASE_URL.'/ajax/Yeelight.html?op=process';
        $data=array(
            "header"=>$pkt->getHeader(),
            "peer"=>$pkt->getPeer(),
            "packet"=>$pkt->getParamsJson()
        );
        $options=array(
            'http'=>array(
